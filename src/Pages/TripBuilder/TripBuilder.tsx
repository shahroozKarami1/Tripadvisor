    ...
    <Box sx={{ width: "100%", p: 2 }}>
      ...
      <Box sx={{ mt: 2 }}>
        {activeStep === steps.length ? (
          <Typography>تبریک! همه مراحل تکمیل شد.</Typography>
        ) : (
          <>
            <Box>
              {activeStep === 1 && <FirstStep  changePageHandler={ handleNext}/>}
              {activeStep === 2 && <SecoundStep />}
              {activeStep === 3 && <ThirdStep />}
              {activeStep === 4 && <FinishStep />}
            </Box>
            <Box sx={{ mt: 4 }}  display={"flex"}  justifyContent={"flex-start"}  gap={1}>
              <Button
